...
#                     "Quotation",
#                     custom_quotation,
#                     ["grand_total", "modified", "custom_quotation_status", "docstatus"],
#                     as_dict=True,
#                 )
#                 if quotation_data:
...
#                     quotation_submitted     = quotation_data.get("docstatus") == 1
...
#             "completed":   bool(custom_quotation) and quotation_submitted,
#             "completedAt": (quotation_issued_at or (str(quotation_modified) if quotation_modified else None)) if quotation_submitted else None,
#             "details": {
#                 "quotationNo": custom_quotation if quotation_submitted else None,
#                 "amount":      quotation_grand_total if quotation_submitted else None,
#                 "remarks":     "Quotation issued to customer" if quotation_submitted else "Quotation not yet issued"
#             }
#         },
...
@frappe.whitelist(allow_guest=False)
def get_work_status(request_id, language="en"):
    """
    GET API to fetch Maintenance Request details with status timeline.
    ...
    def t(value):
        """Translate a string value via _() ; pass through anything else unchanged."""
        if isinstance(value, str) and value:
            return _(value)
        return value

    def translate_details(details):
        return {key: t(val) for key, val in details.items()}
    ...
    # ── Fetch asset_name from Asset doctype — dedicated Arabic field,
    #    same pattern as the other endpoints (this is an actual field
    #    value, not fixed vocabulary, so it doesn't go through _()) ──
    ...
    if doc.asset:
        ...
        asset_doc = frappe.db.get_value("Asset", doc.asset, asset_fields, as_dict=True)
        if asset_doc:
            asset_name = pick(
    ...
    statuses = [
        ...
        {
            "code":        "QUOTATION_APPROVED",
            "title":       _("Quotation Approved") if not quotation_rejected else _("Quotation Rejected"),
            "completed":   quotation_approved,
            "completedAt": str(quotation_modified) if quotation_approved and quotation_modified else None,
            "details": translate_details({
                "approvedBy": "Customer",
    ...
    return {
        ...
        "data": {
            "requestId":   doc.name,
            "title":       _(doc.description) if doc.description else doc.description,
            "requestType": _(doc.maintenance_type) if doc.maintenance_type else doc.maintenance_type,
            "requestDate": str(doc.date_of_submit) if doc.date_of_submit else None,
            "assetName":   asset_name,
            "statuses":    statuses,
            "language":    language,
