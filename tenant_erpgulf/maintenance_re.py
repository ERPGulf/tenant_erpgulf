...
def create_maintenance_log_from_request(maintenance_request):
    ...
    if mr.maintenance_log:
        frappe.throw(
            _("A Maintenance Log ({0}) already exists for this request.").format(mr.maintenance_log)
        )

    is_asset_scope = mr.get("custom_maintenance_scope") == "Asset"

    log = frappe.new_doc("Asset Maintenance Log")
    log.custom_asset_maintenance_type = "Reactive"
    log.custom_maintenance_types      = mr.maintenance_type
    log.maintenance_status            = "Planned"
    log.description                   = mr.description
    log.custom_maintenance_scope      = mr.get("custom_maintenance_scope")

    if is_asset_scope:
        log.custom_asset = mr.asset
    else:
        log.custom_scope_reference = mr.get("custom_scope_reference")
        log.custom_customer        = mr.customer
