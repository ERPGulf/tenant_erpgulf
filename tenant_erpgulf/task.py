...
@frappe.whitelist(allow_guest=False)
def update_employee_task_status(task_id, status, date=None):
    """
    ...
    try:
        ...
        # ── STEP 5: Validate reference type ───────────────────────────────────
        if todo.get("reference_type") != "Asset Maintenance Log":
            return Response(
        ...
        frappe.log_error(
            ...
            message=f"task_id={task_id} | aml={aml_name} | status={db_status} | date={date}"
        )
